...
class M3A(base_agent.EnvironmentInteractingAgent):
  ...
  def step(self, goal: str) -> base_agent.AgentInteractionResult:
    ...
    ################################# mllm预测一步action及后处理 #################################
    action_output, is_safe, raw_response = self.llm.predict_mm(  # 有空看看图像和prompt的关系，即如何插入图像的
        action_prompt,
        [
    ...
    try:
      ...
      if converted_action.action_type == "swipe":
        print("将swipe修正为scroll")
        converted_action.action_type = "scroll"
    ...
    ################################# 执行动作 #################################
    # 获取action对应的操作序号
    print("准备执行的动作为：", converted_action)
