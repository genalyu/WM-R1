...
from android_world.agents.wm_utils import android_world_to_control, viz
from concurrent.futures import ThreadPoolExecutor, as_completed
import time
from android_world.agents.qwen_model import QwenModel

# world model
model_name="/home/zyh/zyh_documents/zla/ckpt/Qwen3-VL-8B-sft-600steps"
models=[
  QwenModel(model_name),
  QwenModel(model_name),
  QwenModel(model_name)
]


def post_figure_action(goal, description, image_array, action, ui_elements, html_save_path, idx):
    # 转成PIL图像
    raw_screenshot = Image.fromarray(image_array)
    acAction = android_world_to_control(action, ui_elements)['arguments']
    if acAction=={}:
       print(f"渲染失败！！！action为{action},ui_element为：{ui_elements}")
       return None
    hint_screenshot = viz.visualize(raw_screenshot, acAction)
    returned_array = models[idx-1].post(goal=goal, description=description, image=hint_screenshot, action=action, html_save_path=html_save_path, idx=idx)
...
ACTION_SELECTION_PROMPT_TEMPLATE = (
    ...
    + textwrap.dedent("""
      ...
      2. Clarification about the semantics of the three actions: the three actions must represent alternative candidate actions to be executed NEXT in the current step (i.e., each is a possible single next action to perform right now). They are not to be interpreted as a sequence of three successive steps.
                      
      3. You must output three candidate actions for the next step in the current step. If you feel there are no other reasonable outputs, you can set the confidence of the remaining actions to 0.1.
                      
      4. Use **double quotes** for all JSON strings and keys (standard JSON). Do NOT use single quotes for JSON.

      5. All string values (including Reason and any Action fields such as text) must be valid JSON strings. If a string contains double quotes, newline characters, backslashes, or other characters that would break JSON, you MUST escape them using standard JSON escaping (for example: a quote becomes `\"`, a backslash becomes `\\`, a newline becomes `\n`).

      6. Do NOT output trailing commas. Do NOT include comments or non-JSON tokens.

      7. Each "Reason" must be a plain string explaining why that action is valid/plausible.  
      ...
      8. Output must be compact or pretty-printed JSON — both are acceptable as long as it is valid JSON.

      9. Add a **Confidence** field for each action:
...
def extract_dictionary_from_string(input_string: str):
    ...
    for key, value in parsed.items():
        ...
        if "action_type" in action and action["action_type"] == "swipe":
...
def remove_tail_repetition(text, min_pattern_len=10, max_pattern_len=500):
    """
    检测并删除字符串尾部的重复文本（包括因 max_tokens 截断导致的残缺重复）。
    """
    ...
    for offset in range(min(n, max_pattern_len)):
        for L in range(min_pattern_len, max_pattern_len):
            if n - offset - 2 * L < 0:
                break
                
            block2 = text[n - offset - L : n - offset]
            block1 = text[n - offset - 2 * L : n - offset - L]
            
            if block1 == block2:
                pattern = block1
                first_idx = text.find(pattern)
                return text[:first_idx + len(pattern)].strip()
...
def remove_trailing_brace_simple(text):
    stripped = text.rstrip()
    if stripped.endswith('}'):
        last_brace = stripped.rfind('{')
        if last_brace != -1:
            original_brace_pos = text.find(stripped[last_brace])
            return text[:original_brace_pos]
    return text
...
class M3A_VM(base_agent.EnvironmentInteractingAgent):
  ...
  def step(self, goal: str) -> base_agent.AgentInteractionResult:
    ...
    ################################# mllm预测一步action及后处理 #################################
    action_output_temp, is_safe, raw_response = self.llm.predict_mm( 
        action_prompt,
        [
    ...
    for act_key in actions_reason.keys():
      # 防止agent输出的confidence是字符串类型
      actions_reason[act_key]['Confidence'] = float(actions_reason[act_key]['Confidence'])
      if actions_reason[act_key]['Confidence']>=0.9:
        cnt+=1


    if len(actions_reason)<3 or \
        actions_reason[1]['Action']['action_type'] in ["answer", "status"] or \
        cnt==1 or actions_reason[1]['Action']==actions_reason[2]['Action']:
        reason = actions_reason[1]['Reason']
        action = actions_reason[1]['Action']
        action_output = f'Reason: {reason}\nAction: {action}'
        step_data['action_output'] = action_output
        step_data['action_raw_response'] = raw_response
        print(f"特殊情况，跳过world model预测，直接选择第一个：", action_output)
      ...
      html_save_path=f"html_tmp/{self.name}/{goal.split()[0]}/{step_index}/"
      os.makedirs(html_save_path, exist_ok=True)
      
      with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
          futures = [
              executor.submit(post_figure_action, goal, action_sums[1], step_data['raw_screenshot'], actions_reason[1]['Action'], before_ui_elements, html_save_path, 1),
              executor.submit(post_figure_action, goal, action_sums[2], step_data['raw_screenshot'], actions_reason[2]['Action'], before_ui_elements, html_save_path, 2),
              executor.submit(post_figure_action, goal, action_sums[3], step_data['raw_screenshot'], actions_reason[3]['Action'], before_ui_elements, html_save_path, 3),
          ]
    ...
    print("解析成功，最终选择的action和reason如下：")
    print('Action: ' + action)
    print('Reason: ' + reason)
    ...
    summary = remove_trailing_brace_simple(summary)
    ...
    summary=remove_tail_repetition(summary)
