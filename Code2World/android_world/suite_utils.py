...
# 创建任务套件，包含一系列任务，为每个任务生成多个（默认为1）实例
def create_suite(
  ...
  for name, task_type in task_registry.items():
    current = []
    # 为每一个任务用不同参数创建n_task_combinations个实例
  ...
  for name, instances in suite.items():
    ...
    for i, instance in enumerate(instances):
      ...
      if already_processed:
        _log_and_print(f"执行任务的目标为：{instance.goal}", )
        _log_and_print('Skipping already processed task %s', instance_name)
        process_episodes_fn(episodes_metadata, print_summary=True)  # 补充打印一遍日志信息
        continue
