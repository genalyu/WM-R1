...
class UIElement:
  ...
  text: Optional[str] = None  # 元素显示的文本
  content_description: Optional[str] = None  # （辅助功能/无障碍文本），常用于语音朗读或辅助识别
  class_name: Optional[str] = None  # UI 控件的类名（如 android.widget.TextView、android.widget.ImageView 等），用于识别控件类型。
  bbox: Optional[BoundingBox] = None  # 归一化或相对坐标的边界框（通常是相对于屏幕宽高的 0~1 值），基本都为 None
  bbox_pixels: Optional[BoundingBox] = None  # 以像素为单位的边界框，形式 BoundingBox(x_min=..., x_max=..., y_min=..., y_max=...)。这是实际用于点击的坐标。
  hint_text: Optional[str] = None  # 输入框的 hint（占位提示），一般用于输入控件。
  is_checked: Optional[bool] = None  # 是否处于勾选状态（复选框或开关）。
  is_checkable: Optional[bool] = None  # 是否可勾选
  is_clickable: Optional[bool] = None  # 是否可点击（可交互）。
  is_editable: Optional[bool] = None  # 是否可输入（文本框等可编辑）。
  is_enabled: Optional[bool] = None  # 是否可用（enabled/disabled）。
  is_focused: Optional[bool] = None  # 是否当前有焦点
  is_focusable: Optional[bool] = None  # 是否可以获得焦点
  is_long_clickable: Optional[bool] = None  # 是否支持长按
  is_scrollable: Optional[bool] = None  # 是否可滚动（ScrollView、列表等）
  is_selected: Optional[bool] = None  # 是否被选中（例如选项卡）
  is_visible: Optional[bool] = None  # 是否可见（是否在可视区域）
  package_name: Optional[str] = None  # 
  resource_name: Optional[str] = None  # Android 的资源 id（有时为 package:id/name），对自动化很重要（比文本更稳健）
  tooltip: Optional[str] = None  # 鼠标/长按提示（Android 很少见）
  resource_id: Optional[str] = None  # 
  metadata: Optional[dict[str, Any]] = None  # 额外元数据，视实现而定。
