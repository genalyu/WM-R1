...
def get_families() -> list[str]:
  return [
      TaskRegistry.ANDROID_WORLD_FAMILY,  # 包含ANDROID_FAMILY和INFORMATION_RETRIEVAL_FAMILY
      TaskRegistry.ANDROID_FAMILY,  # 包含TaskRegistry._TASKS中的所有任务，在init中注册的
      TaskRegistry.MINIWOB_FAMILY,  # 从外部导入的miniwob_registry.TASK_REGISTRY任务集合
      TaskRegistry.MINIWOB_FAMILY_SUBSET,  # 从外部导入的miniwob_registry.TASK_REGISTRY_SUBSET任务集合
      TaskRegistry.INFORMATION_RETRIEVAL_FAMILY,  # 包含从information_retrieval_registry中导入的信息检索相关任务集合
  ]
...
class TaskRegistry:
  ...
  def __init__(self):
    # 初始化时，将所有_TASKS中的任务注册到ANDROID_TASK_REGISTRY
